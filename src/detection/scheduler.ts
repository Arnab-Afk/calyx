...
): Promise<{ servicesChecked: number; alertsDetected: number; deliveriesQueued: number }> {
...
export async function runDeliveryCycle(
  ...
  for (const delivery of deliveries) {
    try {
      const context = await getAlertContext(delivery.tenant_id, delivery.alert_id);
      if (!context) throw new Error("Alert context no longer exists for this tenant");
      const handler = handlers[delivery.destination];
      if (!handler) throw new Error(`No ${delivery.destination} delivery handler configured`);
      const alert: Alert = {
        id: context.id,
        ...
        recommended_action: "Investigate the linked evidence before taking action.",
        anomaly: {
