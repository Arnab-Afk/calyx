...
export async function internalAskRoute(app: FastifyInstance): Promise<void> {
  app.post("/v1/internal/workspaces/:workspaceId/ask", async (request, reply) => {
    ...
    const response = await runAgent(tenantId, parsed.data.message);
    const chartable = [...response.toolCallsMade]
      .reverse()
      .find((call) => call.output?.visualization_hint && call.output.visualization_hint !== "none");
    const chartType = chartable?.output
      ? autoChartType(chartable.output.visualization_hint!, chartable.output.data)
      : null;
    ...
    return {
      ...
      chartType,
      chartData: chartable?.output?.data ?? null,
