...
WS=$(curl -sf -X POST "$BASE/v1/workspaces" \
  ...
  -H 'Content-Type: application/json' \
  -d '{"name":"smoke-ws"}')
WS_ID=$(echo "$WS" | python -c "import sys,json; print(json.load(sys.stdin)['workspace']['id'])")
echo "workspace $WS_ID"

CHS=$(curl -sf "$BASE/v1/workspaces/$WS_ID/channels" -H "Authorization: Bearer $TOKEN")
CH_ID=$(echo "$CHS" | python -c "import sys,json; print(json.load(sys.stdin)['channels'][0]['id'])")
echo "channel $CH_ID"
...
echo "message $(echo "$MSG" | python -c "import sys,json; print(json.load(sys.stdin)['id'])")"
echo "OK"
