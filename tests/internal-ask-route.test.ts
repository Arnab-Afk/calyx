...
vi.mock("../src/storage/workspace-tenants.js", () => ({ tenantForWorkspace: vi.fn() }));
...
describe("trusted internal workspace ask", () => {
  ...
  it("retires the caller-selected tenant endpoint", async () => {
    ...
    const response = await instance.inject({
      ...
      headers: { "x-tenant-id": "attacker-tenant" }, body: { message: "why" },
    });
  ...
  it("rejects requests without the internal credential", async () => {
    ...
    const response = await instance.inject({
      method: "POST", url: "/v1/internal/workspaces/workspace-a/ask", body: { message: "why" },
    });
  ...
  it("rejects workspaces without a canonical tenant link", async () => {
    ...
    const instance = await app();
    const response = await instance.inject({
      method: "POST", url: "/v1/internal/workspaces/workspace-a/ask",
      headers: { "x-calyx-internal-key": "internal-test-key" }, body: { message: "why" },
    });
  ...
  it("runs the agent only under the server-resolved tenant", async () => {
    ...
    agent.mockResolvedValue({
      answer: "Because errors increased.", toolCallsMade: [], turns: 1, stopReason: "end_turn",
    });
    const instance = await app();
    const response = await instance.inject({
      method: "POST", url: "/v1/internal/workspaces/workspace-a/ask",
      headers: { "x-calyx-internal-key": "internal-test-key" },
      body: { message: "why", tenant_id: "attacker-tenant" },
    });
    expect(response.statusCode).toBe(200);
    expect(agent).toHaveBeenCalledWith("tenant-canonical", "why");
    await instance.close();
  });
});
